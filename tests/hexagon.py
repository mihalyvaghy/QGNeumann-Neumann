...
parser = argparse.ArgumentParser(
        prog='QG solver for lobsters',
        description='Run Neumann-Neumann algorithm and QG FEM on hexagonal lattice graphs of various sizes',
        epilog='Bottom text.')
...
parser.add_argument('--N_max', action='store', default=14,
                    help='maximum number of rows and columns', type=int)
parser.add_argument('--N_min', action='store', default=4,
                    help='minimum number of rows and columns', type=int)
parser.add_argument('--N_step', action='store', default=2,
                    help='size number step', type=int)
...
def create_graph(size):
    G = nx.hexagonal_lattice_graph(size, size)
    ...
    coords = list(nx.kamada_kawai_layout(G).values())
    return adj, coords
